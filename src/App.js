...
class App extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    this.setState({
      withID: await this.getProfileData("5fc4ae95b708c200175de88d"),
    })
  }

  componentDidUpdate = async (_previousProps, _previousState) => {
    if (this.state.me._id !== undefined && this.state.exp === null)
      this.setState({ exp: await this.getExperiences(this.state.me._id) })
  }
  ...
  putExperienceData = async (userID, data, expID) => {
    ...
    try {
      if (userID === "" || userID === undefined || userID === null)
        throw new Error("user id must be present")
      if (expID === "" || expID === undefined || expID === null)
        throw new Error("experience id must be present")
      if (typeof data !== "object") throw new Error("data must be an object")
      results = await fetch(
        this.state.endpoint + userID + "/experiences/" + expID,
        {
          method: "PUT",
          headers: {
            Authorization: this.state.authtoken,
  ...
  getProfileData = async (id) => {
    ...
    try {
      results = await fetch(
        this.state.endpoint + (id !== undefined ? id : ""),
        {
          headers: {
            Authorization: this.state.authtoken,
          },
